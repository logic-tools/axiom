(* Anonymous ESSLLI 2020 Student Session Submission *)
...
definition hintikka :: \<open>form set \<Rightarrow> bool\<close> where
  \<open>hintikka H \<equiv>
    \<^bold>\<bottom> \<notin> H \<and>
    (\<forall>n. Pro n \<in> H \<longrightarrow> (\<^bold>\<not> Pro n) \<notin> H) \<and>
    (\<forall>p q. (p \<^bold>\<longrightarrow> q) \<in> H \<longrightarrow> (\<^bold>\<not> p) \<in> H \<or> q \<in> H) \<and>
    (\<forall>p q. (\<^bold>\<not> (p \<^bold>\<longrightarrow> q)) \<in> H \<longrightarrow> p \<in> H \<and> (\<^bold>\<not> q) \<in> H)\<close>

abbreviation (input) \<open>model H n \<equiv> Pro n \<in> H\<close>

lemma hintikka_model: \<open>hintikka H \<Longrightarrow> (p \<in> H \<longrightarrow> model H \<Turnstile> p) \<and> ((\<^bold>\<not> p) \<in> H \<longrightarrow> \<not> model H \<Turnstile> p)\<close>
...
next
  fix n
  assume \<open>Pro n \<in> S\<close> \<open>(\<^bold>\<not> Pro n) \<in> S\<close>
...
next
  fix p q
  assume *: \<open>(p \<^bold>\<longrightarrow> q) \<in> S\<close> \<open>q \<notin> S\<close>
  ...
  show \<open>(\<^bold>\<not> p) \<in> S\<close>
  proof (rule ccontr)
    ...
    moreover have \<open>set ((p \<^bold>\<longrightarrow> q) # S') \<subseteq> S\<close>
...
next
  fix p q
  assume *: \<open>(\<^bold>\<not> (p \<^bold>\<longrightarrow> q)) \<in> S\<close>
  show \<open>(\<^bold>\<not> q) \<in> S\<close>
  proof (rule ccontr)
