...
theory CICM imports Main begin
...
section \<open>System L1 from Jan Lukasiewicz: Elements of Mathematical Logic (English Tr. 1963)\<close>
...
theorem L1_soundness: \<open>\<forall>I. (I \<Turnstile> p)\<close> if \<open>\<turnstile> p\<close>
...
lemmas Clav = Axiomatics.intros(3)

lemmas Expl = Axiomatics.intros(4)
...
lemma thes15: \<open>\<turnstile> ((\<sim> p \<rightarrow> q) \<rightarrow> ((q \<rightarrow> p) \<rightarrow> p))\<close>
  using MP Clav thes14 by metis

lemma thes16: \<open>\<turnstile> (p \<rightarrow> p)\<close>
  using MP thes09 Clav by metis
...
lemma thes18: \<open>\<turnstile> (q \<rightarrow> (p \<rightarrow> q))\<close>
  using MP Expl thes05 thes17 by metis
...
lemma thes24: \<open>\<turnstile> ((((p \<rightarrow> q) \<rightarrow> p) \<rightarrow> p))\<close>
  using MP Expl thes15 thes23 by metis
...
theorem L1_completeness: \<open>\<turnstile> p\<close> if \<open>\<forall>I. (I \<Turnstile> p)\<close>
  using that imply_completeness[where ps=\<open>[]\<close>] by simp

section \<open>The Meredith Single Axiom (C. A. Meredith, J. Computing Systems 1:155-164 1953)\<close>
...
inductive Single (\<open>\<tturnstile>\<close>) where
  ...
  \<open>\<tturnstile> (((((p \<rightarrow> q) \<rightarrow> (\<sim> r \<rightarrow> \<sim> s)) \<rightarrow> r) \<rightarrow> t) \<rightarrow> ((t \<rightarrow> p) \<rightarrow> (s \<rightarrow> p)))\<close>

lemma L1_3: \<open>\<tturnstile> (p \<rightarrow> (\<sim> p \<rightarrow> q))\<close>
  using Single.intros by metis
...
lemma 2: \<open>\<tturnstile> (p \<rightarrow> (((q \<rightarrow> p) \<rightarrow> r) \<rightarrow> (s \<rightarrow> r)))\<close>
  using Single.intros 1 by metis
...
lemma 3: \<open>\<tturnstile> (p \<rightarrow> (((q \<rightarrow> r) \<rightarrow> s) \<rightarrow> (((r \<rightarrow> t) \<rightarrow> (\<sim> s \<rightarrow> \<sim> q)) \<rightarrow> s)))\<close>
  using Single.intros 1 2 by meson
